...
from earthgazer.cli.services import run_discover_workflow
from earthgazer.cli.services import run_discovery_and_backup_workflow
from earthgazer.cli.services import run_full_pipeline_workflow
from earthgazer.cli.services import run_location_backup_workflow
from earthgazer.cli.services import run_location_workflow
from earthgazer.cli.services import run_multiple_captures_workflow
from earthgazer.cli.services import run_reprocess_workflow
from earthgazer.cli.services import run_single_capture_workflow
from earthgazer.cli.utils import follow_task
from earthgazer.cli.utils import parse_bounds
...
def discover(follow_flag, output_json):
    ...
    if output_json:
        ...
        console.print("[green]Discovery workflow started[/green]")
...
def pipeline(follow_flag, output_json):
    ...
    if not output_json:
        console.print("[yellow]Warning: Full pipeline can take a long time to complete.[/yellow]")
    ...
    if output_json:
        ...
        console.print("[green]Full pipeline workflow started[/green]")
        console.print(f"Task ID: {task_id}")
        console.print("\nThis workflow will:")
...
def process_multiple(capture_ids, bands, bounds, temporal_analysis, force, follow_flag, output_json):
    ...
    # Start workflow
    try:
        task_id = run_multiple_captures_workflow(capture_id_list, bands_list, bounds_tuple, temporal_analysis, force)
    except Exception as e:
        console.print(f"[red]Error starting workflow: {e}[/red]", stderr=True)
        raise click.Abort from None

    if output_json:
        result = {"task_id": task_id, "capture_ids": capture_id_list, "workflow": "process_multiple", "force": force}
        click.echo(format_json(result))
    else:
...
def full_pipeline(location_ids, bands, bounds, mission, force, follow_flag, output_json):
    ...
    if not output_json:
        console.print("[yellow]Warning: Full pipeline can take a very long time to complete.[/yellow]")

    # Start workflow
    try:
        task_id = run_full_pipeline_workflow(location_id_list, bands_list, bounds_tuple, mission, force)
    except Exception as e:
        console.print(f"[red]Error starting workflow: {e}[/red]", stderr=True)
        raise click.Abort from None
...
def reprocess(mission, bands, bounds, limit, force, follow_flag, output_json):
    ...
    # Start workflow
    try:
        task_id = run_reprocess_workflow(mission, bands_list, bounds_tuple, limit, force)
    except Exception as e:
        console.print(f"[red]Error starting workflow: {e}[/red]", stderr=True)
        raise click.Abort from None
    ...
    if output_json:
        ...
        console.print("[green]Reprocessing workflow started[/green]")
        console.print(f"Task ID: {task_id}")
        console.print(f"Bands: {bands_list}")
        if mission:
...
def process_location(location_id, bands, mission, limit, temporal_analysis, force, follow_flag, output_json):
    ...
    # Start workflow
    try:
        task_id = run_location_workflow(location_id, bands_list, mission, limit, temporal_analysis, force)
    except Exception as e:
        console.print(f"[red]Error starting workflow: {e}[/red]", stderr=True)
        raise click.Abort from None
    ...
    if output_json:
        result = {"task_id": task_id, "location_id": location_id, "workflow": "process_location", "force": force}
        click.echo(format_json(result))
    else:
...
def backup_location(location_id, mission, limit, follow_flag, output_json):
    ...
    # Start workflow
    try:
        task_id = run_location_backup_workflow(location_id, mission, limit)
    except ValueError as e:
        console.print(f"[red]Error: {e}[/red]", stderr=True)
        raise click.Abort from None
    except Exception as e:
    ...
    if output_json:
        result = {"task_id": task_id, "location_id": location_id, "workflow": "backup_location"}
        click.echo(format_json(result))
    else:
