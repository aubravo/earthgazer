...
def get_system_status() -> dict[str, Any]:
    ...
    try:
        ...
        with Session(engine) as session:
            ...
            status["backed_up"] = session.query(CaptureData).filter(CaptureData.backed_up).count()
...
def get_task_history(limit: int = 50) -> list[dict[str, Any]]:
    ...
    try:
        ...
        with Session(engine) as session:
            executions = session.query(TaskExecution).order_by(TaskExecution.created_at.desc()).limit(limit).all()
...
def get_captures(backed_up_only: bool = False, limit: int = 100) -> list[dict[str, Any]]:
    ...
    try:
        ...
        with Session(engine) as session:
            ...
            if backed_up_only:
                query = query.filter(CaptureData.backed_up)
...
def create_location(
    name: str, min_lon: float, min_lat: float, max_lon: float, max_lat: float, from_date: str, to_date: str, active: bool = True
) -> int:
...
def run_discover_workflow(location_ids: list[int] | None = None) -> str:
...
def run_backup_workflow(capture_ids: list[int] | None = None) -> str:
...
def run_single_capture_workflow(capture_id: int, bands: list[str] = None, bounds: tuple = None, force: bool = False) -> str:
    ...
    result = process_single_capture_workflow(capture_id=capture_id, bands=bands, bounds=bounds, force=force)
    return result.id


def run_discovery_and_backup_workflow(location_ids: list[int] | None = None) -> str:
...
def run_multiple_captures_workflow(
    capture_ids: list[int], bands: list[str] = None, bounds: tuple = None, run_temporal_analysis: bool = True, force: bool = False
) -> str:
    ...
    result = process_multiple_captures_workflow(
        capture_ids=capture_ids, bands=bands, bounds=bounds, run_temporal_analysis=run_temporal_analysis, force=force
    )
    return result.id
...
def run_full_pipeline_workflow(
    ...
    result = full_pipeline_workflow(location_ids=location_ids, bands=bands, bounds=bounds, mission_filter=mission_filter, force=force)
    return result.id if result else None


def run_reprocess_workflow(
    mission_filter: str | None = None, bands: list[str] = None, bounds: tuple = None, limit: int | None = None, force: bool = False
) -> str | None:
    ...
    result = reprocess_existing_captures_workflow(mission_filter=mission_filter, bands=bands, bounds=bounds, limit=limit, force=force)
    return result.id if result else None
...
def run_location_backup_workflow(location_id: int, mission_filter: str | None = None, limit: int | None = None) -> str | None:
    """
