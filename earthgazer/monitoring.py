...
from earthgazer.database.definitions import TaskStatus
...
def get_capture_id_from_task(task, args, kwargs) -> int | None:
    """
        ...
        with Session(get_engine()) as session:
            ...
            if existing:
                ...
                task_exec = TaskExecution(
                    task_id=task_id, task_name=task.name, capture_id=capture_id, status=TaskStatus.STARTED, started_at=datetime.now()
                )
...
def task_retry_handler(sender=None, task_id=None, reason=None, einfo=None, **extra):
    ...
    """
    logger.warning(
        f"Task {sender.name}[{task_id}] retrying due to: {reason}",
        extra={
            "task_id": task_id,
            "task_name": sender.name,
            "reason": str(reason),
...
def task_revoked_handler(sender=None, request=None, terminated=None, signum=None, expired=None, **extra):
    ...
    logger.warning(
        f"Task {task_name}[{task_id}] revoked",
        extra={
            "task_id": task_id,
            "task_name": task_name,
